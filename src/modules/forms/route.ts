...
formRouter.get('/:slug', authMiddleware as any, async (req, res) => {
...
formRouter.post('/:slug/submit', authMiddleware as any, async (req: any, res) => {
  ...
  for (const r of required) if (!(r in answers)) return res.status(400).json({ message: `Missing ${r}` });
